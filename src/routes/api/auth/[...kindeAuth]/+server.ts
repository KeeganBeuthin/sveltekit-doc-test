...
export async function GET(event: RequestEvent) {
  ...
  // (same method used in kindeCloudflareStorage.ts)
  ...
  const POST_LOGIN_REDIRECT_URL = env.KINDE_POST_LOGIN_REDIRECT_URL 
  const POST_LOGOUT_REDIRECT_URL = env.KINDE_POST_LOGOUT_REDIRECT_URL
  const SCOPE = env.KINDE_SCOPE 
  const USE_PKCE = env.KINDE_AUTH_WITH_PKCE 
  ...
  // The rest of the functions remain unchanged and use closures to access the env variables
